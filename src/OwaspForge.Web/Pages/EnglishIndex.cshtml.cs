...
public sealed class EnglishIndexModel(EnglishChallengeRegistry registry, ProgressService progressService) : PageModel
{
    ...
    public async Task OnGetAsync(CancellationToken cancellationToken) { Challenges = registry.All; CompletedIds = await progressService.CompletedIdsAsync(cancellationToken); }
}
